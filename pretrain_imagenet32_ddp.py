...
def train(rank, world_size):
    ...
    train_loader = MultiEpochsDataLoader(train_dataset,
            batch_size=FLAGS.BATCH_SIZE//FLAGS.N_GPUS,
    ...
    model = resnet50(num_classes=1000).cuda(rank)
    ...
    def train_epoch():
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            if rank == 0 and i%1000 == 0:
                loss = torch.distributed.reduce(loss.detach().cpu(), rank) / FLAGS.N_GPUS
                tprint(f'[TRAIN][{i}/{len(train_loader)}] LR {lr_scheduler.get_last_lr()[-1]:.2e} | loss {loss.item():.3f}')
