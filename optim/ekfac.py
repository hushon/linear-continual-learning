...
class EKFAC(Optimizer):
    ...
    def __init__(self, net, eps, sua=False, ra=False, update_freq=1,
        ...
        for mod in net.modules():
            ...
            if mod_class in ['Linear', 'Conv2d', 'CustomLinear', 'CustomConv2d']:
                ...
                if mod_class in ['CustomLinear', 'CustomConv2d']:
                    params = [mod.weight_tangent]
                    if mod.bias_tangent is not None:
                        params.append(mod.bias_tangent)
                d = {'params': params, 'mod': mod, 'layer_type': mod_class}
                if mod_class in ('Conv2d', 'CustomConv2d'):
                    if not self.sua:
    ...
    def step(self, update_stats=True, update_params=True):
        ...
        for group in self.param_groups:
            ...
            # Preconditionning
            if group['layer_type'] in ('Conv2d', 'CustomConv2d') and self.sua:
                if self.ra:
    ...
    def _forward_hook(self, mod, input, output):
        if mod.__class__.__name__ in ('Linear', 'Conv2d'):
            input, = input
            output = output
        elif mod.__class__.__name__ in ('CustomLinear', 'CustomConv2d'):
            input, _ = input # primal input
            _, output = output # tangent output (=jvp)
        else:
            raise NotImplementedError
        self.state[mod]['x'] = input
        def _tensor_backward_hook(grad: torch.Tensor) -> None:
            self.state[mod]['gy'] = grad[0] * grad[0].size(0)
